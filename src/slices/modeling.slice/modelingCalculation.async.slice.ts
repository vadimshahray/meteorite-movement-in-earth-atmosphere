...
>('modeling/calculateMeteoriteMovement', async (_, { getState }) => {
  ...
  // prettier-ignore
  const k1 = (-h * ((Math.PI * r * r) / m / ((g0 * Rp) / (Rp + H))) * (1.22 * Math.pow(Math.E, -0.000129 * H)) * v * v) / 2 - ((g0 * Rp) / (Rp + H)) * sin(O)

  const m1 = h * cos(O) * (v / (Rp + H) - (g0 * Rp) / (Rp + H) / v)

  const l1 = h * v * sin(O)
  // prettier-ignore
  const k2 = (-h * ((Math.PI * r * r) / m / ((g0 * Rp) / (Rp + H + 0.5 * l1))) * (1.22 * Math.pow(Math.E, -0.000129 * (H + 0.5 * l1))) * (v + 0.5 * k1) * (v + 0.5 * k1)) / 2 - ((g0 * Rp) / (Rp + H + 0.5 * l1)) * sin(O + 0.5 * m1)

  // prettier-ignore
  const m2 = h * cos(O + 0.5 * m1) * ((v + 0.5 * k1) / (Rp + H + 0.5 * l1) - (g0 * Rp) / (Rp + H + 0.5 * l1) / (v + 0.5 * k1))

  // prettier-ignore
  const l2 = h * (v + 0.5 * k1) * sin(O + 0.5 * m1)
  // prettier-ignore
  const k3 = (-h * ((Math.PI * r * r) / m / ((g0 * Rp) / (Rp + H + 0.5 * l2))) * (1.22 * Math.pow(Math.E, -0.000129 * (H + 0.5 * l2))) * (v + 0.5 * k2) * (v + 0.5 * k2)) / 2 - ((g0 * Rp) / (Rp + H + 0.5 * l2)) * sin(O + 0.5 * m2)
  // prettier-ignore
  const m3 = h * cos(O + 0.5 * m2) * ((v + 0.5 * k2) / (Rp + H + 0.5 * l2) - (g0 * Rp) / (Rp + H + 0.5 * l2) / (v + 0.5 * k2))

  const l3 = h * (v + 0.5 * k2) * sin(O + 0.5 * m2)
  // prettier-ignore
  const k4 = (-h * ((Math.PI * r * r) / m / ((g0 * Rp) / (Rp + H + l3))) * (1.22 * Math.pow(Math.E, -0.000129 * (H + l3))) * (v + k3) * (v + k3)) / 2 - ((g0 * Rp) / (Rp + H + l3)) * sin(O + m3)
  // prettier-ignore
  const m4 = h * cos(O + m3) * ((v + k3) / (Rp + H + l3) - (g0 * Rp) / (Rp + H + l3) / (v + k3))
